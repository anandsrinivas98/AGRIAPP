...
class LabourSchedulingService {
  ...
  async checkUpcomingTasks() {
    try {
      ...
      // Neon DB may be in auto-suspend — log and skip, don't crash the process
      console.warn('checkUpcomingTasks: DB unavailable, skipping run.', error?.message);
    }
  }
  ...
  async predictLaborShortages() {
    try {
      ...
      console.warn('predictLaborShortages: DB unavailable, skipping run.', error?.message);
    }
  }
